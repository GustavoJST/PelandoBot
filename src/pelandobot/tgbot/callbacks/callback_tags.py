    ...
    if await async_db.redis.exists(f"state.{call.message.chat.id}"):
        ...
        if call.data == "add_tags":
            await bot.send_message(
                ...
                "5 - Lembre-se de digitar as tags corretamente. "
                "Evite erros como smartfone, notebok, etc.\n\n"
                "6 - Você pode usar no máximo 30 tags ao mesmo tempo.\n\n"
                "OBS: As tags seguem o mesmo sistema do pelando. "
                "Eu irei lhe entregar apenas promoções que "
                "tiverem em seu título uma das tags escolhidas.\n\n"
                "OBS²: Não seguir as regras acima fará com "
                "que você não receba promoções de forma correta "
            ...
            await bot.send_message(
                call.message.chat.id,
                f"[{user_name}](tg://user?id={user_id}), digite quais tags você quer adicionar ou "
                "digite /exit para sair.\n\n"
                "Lembre-se, caso você digite uma tag que já exista na sua "
                "lista de tags, ela será ignorada.",
                parse_mode="Markdown",
            ...
            await bot.send_message(
                ...
                "digite as tags que você quer remover ou digite /exit para sair.\n\n"
                "OBS: Tags digitadas aqui e que não fazem parte da sua "
                "lista de tags atuais serão ignoradas.",
            ...
            await bot.send_message(
                call.message.chat.id,
                "Tags limpas com sucesso. " "Você receberá todas as novas promoções a partir de agora.",
            )
            return
...
async def handle_tags_input(message: Message, bot: AsyncTeleBot):
    ...
    user_state_string: str = await async_db.redis.get(f"state.{message.chat.id}")
    user_state: str = user_state_string.split('"')[5].split(":")[1]

    # User reply state key in db has expired.
    if user_state is None:
        await bot.send_message(
            message.chat.id,
            "Tempo limite para a configuração de tags atingido. "
    ...
    if not re.search(r"(?i)^[a-zà-ãç-íò-õù-ú0-9\ ,]+$", message.text) or re.search(
        r"(^|\s|\,)\w(\s|\,|$)", message.text
    ):
        await bot.reply_to(
            message,
            "Suas tags não seguem o padrão necessário. "
    ...
    if user_state == "tags_add":
        ...
        if not add_tags_difference:
            await bot.reply_to(
                message,
                "Todas as tags digitadas já estão presentes na sua lista de tags atuais. "
        ...
        if (len(add_tags_difference) + len(db_tags)) > 30:
            await bot.reply_to(
                message,
                "Você está ultrapassando o limite de 30 tags. "
        ...
        await bot.reply_to(
            message,
            f"Operação concluída. {len(add_tags_difference)} tag(s) definida(s) com sucesso e "
        ...
        if not rem_tags_difference:
            await bot.reply_to(
                message,
                "Nenhuma das tags digitadas está presente nas suas tags atuais. Digite novamente ou "
                "digite /exit para sair.",
            )
            return
        ...
        await bot.send_message(
            message.chat.id,
            f"Operação concluída. {len(rem_tags_difference)} tag(s) removida(s) com sucesso e "
