...
async def start(message: Message, bot: AsyncTeleBot):
    if await async_db.redis.scard("active.chats.id") >= 1000 and not await async_db.redis.sismember(
        "active.chats.id", message.chat.id
    ):
        bot.send_message(
            message.chat.id,
            "Olá. Eu sou o PelandoBot, um bot não oficial do Pelando.\n\n"
            "Desculpe, mas não podemos incluir você na lista "
            "de membros pois ela está atualmente cheia.\n\n"
    ...
    await bot.send_message(
        message.chat.id,
        "Olá. Eu sou o PelandoBot, um bot de monitoramento de promoções não oficial do Pelando.\n\n"
