...
async def help(message: Message, bot: AsyncTeleBot):
    await bot.send_message(
        ...
        "* Filtrar as novas promoções, entregando apenas as relevantes "
        "para você através do sistema de Tags.\n\n"
        "<b>O que são tags?</b>\n\n"
        "Tags são palavras-chave que são usadas para filtrar promoções. "
        "No site do Pelando, você tem a opção de escolher tags para receber "
        "novos alertas referentes as tags escolhidas.\n"
        "Por exemplo, ao escolher a tag 'notebook', você irá receber uma notificação "
        "do aplicativo do Pelando para "
        ...
        "Passado esse tempo, suas tags serão deletadas automaticamente, e será necessário "
        "reconfigurá-las novamente utilizando o comando /tags.\n\n"
        "<b>Configurei minhas tags mas não estou recebendo nenhuma promoção. O que eu faço?</b>\n\n"
        "Duas coisas podem estar acontecendo:\n\n"
        "1 - Você configurou as tags de forma certa, "
        "porém deu azar de nenhuma promoção ter aparecido ainda. "
        "Isso é mais comum do que parece, "
        "além de ser algo que não tem como resolver a não ser esperar.\n\n"
        ...
        "Digite /tags novamente para analisar quais as suas tags atuais, "
        "e remova-as/adicione-as novamente se necessário.",
    )
    ...
    await bot.send_message(
        ...
        "Infelizmente, o Telegram possui um limite na quantidade de requisições "
        "que podem ser feita aos seus "
        "servidores em um determinado espaço de tempo. Atualmente, "
        "esse limite é de 30 requisições (envios de mensagem) "
        "por segundo, o que torna a tarefa de mandar promoções atualizadas algo difícil, "
        "pois quanto mais usuários, "
        "maior será o delay entre uma nova promoção surgir e eu entregar a notificação para você.\n\n"
        "Um jeito eficiente de contornar este problema é me adicionar a um grupo. Dessa forma, "
        "eu consigo alcançar um público "
        "maior enviando apenas uma notificação, ao invés de notificar cada usuário separadamente "
        "por mensagens privadas.\n\n"
        "<b>OBS</b>: Apenas administradores/donos de grupos podem interagir "
        "comigo através dos comandos.\n\n"
